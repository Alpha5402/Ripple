  ...
  <dialog ref="dialog" class="embedding-dialog" aria-labelledby="embedding-heading" @cancel.prevent="close" @close="close">
    ...
    <form @submit.prevent="connect">
      <fieldset :disabled="connecting || state.indexing">
        ...
        <label>API Key（无认证的本地服务可留空）<input v-model="form.apiKey" type="password" autocomplete="off" spellcheck="false" placeholder="仅在本次会话中使用"/></label>
    ...
    <section class="embedding-progress" aria-live="polite">
      ...
      <template v-if="configured"><progress :value="completed" :max="Math.max(1,state.documents.length)"/><p>{{ completed }} / {{ state.documents.length }} 篇笔记 · {{ coverage.readyUnits }} 个片段已索引</p><button type="button" class="primary-button" :disabled="connecting || (settingsChanged && !state.indexing)" @click="index">{{ state.indexing ? '停止索引' : coverage.readyUnits ? '更新索引' : '开始索引' }}</button></template>
      ...
      <p v-if="state.mode === 'public'" class="embedding-hint">浏览器中的笔记、连接和索引仅保留到页面关闭或刷新。需要持久保存可使用桌面版。</p>
    </section>
