...
import type { GlobalGraph, WorkbenchBridge, WorkbenchState, ReadingDocument, HostCommand, EvidenceResult } from '../host/contract.js';
...
watch(() => [mode.value, state.value?.label, state.value?.coverage.indexRevision, state.value?.indexing], () => {
...
async function showEvidence(id: string) {
  const relation = (mode.value === 'global' ? globalGraph.value?.relations : state.value?.current?.snapshot.candidateSet)?.find(r => r.id === id); if (!relation) return;
