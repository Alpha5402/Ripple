...
const embeddingSettings = ref(false);
...
async function chooseFolder() { if (!props.bridge.chooseFolder) return; if (dirty.value) { pendingNavigation = chooseFolder; draftDialog.value = true; return; } try { if (await props.bridge.chooseFolder(readonlyOpen.value)) { editing.value = false; reading.value = undefined; error.value = ''; await reload(); } } catch (cause) { error.value = (cause as Error).message; } }
...
<template>
  <Welcome v-if="!state?.documents.length" :can-open="!!bridge.chooseFolder" :error="error" @open="chooseFolder"/>
  <div v-else class="workbench" :class="{ 'sidebar-hidden': !sidebar, desktop: state?.mode === 'desktop', 'is-exploring': mode === 'explore' }">
