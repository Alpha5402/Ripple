...
import type { WorkbenchBridge, WorkbenchState, ReadingDocument, HostCommand, EvidenceResult } from '../host/contract.js';
...
const mode = ref<'reading' | 'explore'>('reading');
...
async function showEvidence(id: string) {
  const relation = state.value?.current?.snapshot.candidateSet.find(r => r.id === id); if (!relation) return;
...
async function chooseFolder() { if (!props.bridge.chooseFolder) return; if (await props.bridge.chooseFolder(readonlyOpen.value)) { editing.value = false; reading.value = undefined; await reload(); } }
...
onMounted(() => { resized(); sidebar.value = !mobile.value && !new URLSearchParams(location.search).has('embed'); void reload(); unsubscribe = props.bridge.subscribe(() => { void reload(); }); window.addEventListener('keydown', keyboard); window.addEventListener('beforeunload', preventUnload); window.addEventListener('resize', resized); });
...
<template>
  <div class="workbench" :class="{ 'sidebar-hidden': !sidebar, desktop: !!bridge.chooseFolder, 'is-exploring': mode === 'explore' }">
    <aside :inert="draftDialog" class="library-sidebar" aria-label="知识目录" v-show="sidebar">
      ...
      <div class="sidebar-brand"><svg viewBox="0 0 32 32" width="28" height="28" fill="none" aria-hidden="true"><path d="M3 18a13 13 0 0 1 26 0M8 19a8 8 0 0 1 16 0M13 20a3 3 0 0 1 6 0" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/></svg><strong>Ripple</strong><span class="brand-dot"/></div>
      ...
      <template v-if="state?.current && reading">
        <div class="workspace-content">
          <section class="primary-panel">
            ...
            <LocalGraph v-else :state="state" :selected="selectedRelation?.id" @preview="showPreview" @focus="focus" @evidence="showEvidence" @view="bridge.command({ type: 'view', ...$event }).catch(e => error = e.message)"/>
          </section>
        ...
        <footer class="lens-dock"><div class="lens-intro"><span class="lens-symbol"><Icon name="graph" :size="21"/></span><div><strong>Knowledge Lens</strong><span>调节关联的可见范围</span></div></div><div class="lens-range"><div class="lens-labels"><span>聚焦</span><span class="lens-value">{{ lens }} <small>/ 100</small></span><span>拓展</span></div><input type="range" min="0" max="100" step="1" :value="lens" :style="{ '--range-progress': `${lens}%` }" aria-label="Knowledge Lens，从聚焦到拓展" :aria-valuetext="`拓展程度 ${lens}，关系阈值 ${Math.round((1 - lens / 100) * 100)}`" @input="setLens(Number(($event.target as HTMLInputElement).value))"/></div><div class="lens-count"><strong>{{ state.visible?.relations.length ?? 0 }} <span>个关联</span></strong><small>强度 ≥ {{ Math.round((state.visible?.threshold ?? 0) * 100) }}</small></div></footer>
      </template>
      <div v-else class="welcome"><svg width="110" height="110" viewBox="0 0 32 32" fill="none"><path d="M3 18a13 13 0 0 1 26 0M8 19a8 8 0 0 1 16 0M13 20a3 3 0 0 1 6 0" stroke="currentColor" stroke-width="1.1" stroke-linecap="round"/></svg><span class="eyebrow">A SPACE FOR CONNECTED THINKING</span><h1>让知识，自然相连。</h1><p>打开你的 Markdown 目录。<br/>从一篇笔记出发，发现已经存在的关联。</p><button v-if="bridge.chooseFolder" class="primary-button" @click="chooseFolder"><Icon name="folder" :size="18"/>打开知识目录</button><p v-else>正在载入公开知识库…</p><small>本地优先 · 原文保真 · 每一条关联都有证据</small></div>
    </main>
