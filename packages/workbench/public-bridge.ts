...
export function createPublicBridge(bundle: PublicBundle, options: { initial?: BrowserSnapshot; save?: (snapshot: BrowserSnapshot) => Promise<void> } = {}): PersistentPublicBridge {
  ...
  const snapshot = (): BrowserSnapshot => ({ kernel: snapshotService.kernel.exportState(), session: session.exportState(), ...(preferences ? { preferences: { ...preferences, autoIndex } } : {}) });
...
export async function loadPublicBridge(): Promise<WorkbenchBridge> {
  ...
  async function syncSources(workspace: BrowserWorkspace, sources: { path: string; markdown: string }[]): Promise<void> {
    ...
    const local = new Map(bridge.snapshot().kernel.documents.map(d => [d.path, d]));
  ...
  async function synchronize(requestPermission = false): Promise<void> {
    ...
    try {
      ...
      const sources = await readDirectory(workspace.handle!);
      if (!switching && active?.id === workspace.id) await syncSources(workspace, sources);
  ...
  async function choose(target?: BrowserWorkspace): Promise<boolean> {
    ...
    if (picker) {
      try {
        ...
        const sources = await readDirectory(handle);
        let match = target;
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      input.onchange = async () => {
        try {
          const files = [...(input.files ?? [])].filter(file => /\.(md|markdown)$/i.test(file.name) && file.name.toLowerCase() !== 'agents.md' && !(file.webkitRelativePath || file.name).split('/').some(part => part.startsWith('.')));
          ...
          const label = files[0]!.webkitRelativePath.split('/')[0] || '我的笔记';
  ...
  return {
    ...
    async command(command) {
      ...
      const result = await current.command(command);
