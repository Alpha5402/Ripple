...
3. 点击「测试并连接」。首次连接只发送固定测试文本；已开启自动更新的工作区更换配置后会补齐待索引内容。成功后显示实际模型和地址。失败替换保留旧连接与索引。
4. 点击「开始索引」，查看完成笔记数和片段数。支持停止、失败重试、向量复用；完成后自动刷新探索关系。
5. 点击「语义相似」的关系分数，查看两端原文证据。

表单目前提供纯文本索引；多模态仍使用下文的文件配置。桌面 API Key 通过 Electron safeStorage 的系统加密能力单独保存；加密不可用时只保留到进程关闭。浏览器 API Key 仅保留在页面内，重开后可阅读缓存，但更新认证模型前需重新输入 Key。凭据不写入内核缓存或公开导出。桌面 SQLite 与浏览器 IndexedDB 均持久保存向量和已验证的模型配置，重启无需模型探测即可使用旧索引；后续响应仍验证模型身份。分块或模型身份改变会使用对应的向量空间，自动更新开关控制后续增量工作。
